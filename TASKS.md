...
- [ ] Criar estrutura de rotas: `/login`, `/root/*`, `/org/:orgId/*`, `/org/:orgId/workspace/:wsId/*`
...
- [ ] Confirmar que listagem de usuários usa `useQuery` do Convex — atualiza sem reload
...
- [ ] Alterar `POST /v1/auth/login` para consultar `org_settings.loginMethods` do usuário e rejeitar com 403 se `email_password` não está habilitado
...
- [ ] Escrever teste: `POST /v1/auth/login` com `loginMethods` sem `email_password` retorna 403 com reason `method_disabled`
